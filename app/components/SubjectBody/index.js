...
const renderIconSubject = (Icon) => {
  if (Icon === 'Huruf A' || Icon === 'Video Huruf A') {
    return (
      <IconLetterA height={60} width={60}/>
    );
  } else if (Icon === 'Huruf Y' || Icon === 'Video Huruf Y') {
    return (
      <IconLetterY height={60} width={60}/>
    );
  } else if (Icon === 'Huruf Z' || Icon === 'Video Huruf Z') {
    return (
      <IconLetterX height={60} width={60}/>
    );
  } else if (Icon === 'Nama Hewan') {
    return (
      <IconAnimal height={60} width={60}/>
    );
  } else if (Icon === 'Nama Buah') {
    return (
      <IconFruit height={60} width={60}/>
    );
  } else if (Icon === 'Kisah Naga') {
    return (
      <IconDragon height={60} width={60}/>
    );
  } else if (Icon === 'Penjumlahan Dasar') {
    return (
      <IconAddition height={60} width={60}/>
    );
  } else if (Icon === 'Logika Dasar') {
    return (
      <IconBasicLogic height={60} width={60}/>
    );
  } else if (Icon === 'Video Viral Bagas Dribble') {
    return (
      <IconBasketball height={60} width={60}/>
    );
  } else {
    return (
      <IconNoImage height={60} width={60}/>
    );
  }
}
const SubjectBody = ({subject}) => {
    return (
        <ScrollView className="flex-1 h-full w-full bg-white px-4 ">
            <Text className="text-xl font-semibold text-black mb-4">{subject}</Text>
            <View className="flex flex-row flex-wrap justify-center gap-7">
              {material.map((item) => {if(item.subject === subject){
                return (
                  <View key={item.id}>
                    <TouchableOpacity onPress={() => router.push({pathname: "../../../(tabs)/libraryScreen/[subjectScreen]/[materialScreen]", params: {material: item.name}})} activeOpacity={0.7} className={`h-24 w-40 my-2 px-4 py-6 rounded-3xl bg-[#FFFFFF] shadow-lg shadow-black`}>
                      <View className="mx-8 mb-2 h-[60px] w-[60px]">
                        {renderIconSubject(item.name)}
                      </View>
